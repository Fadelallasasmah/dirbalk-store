...
| ADR-022 | 2026-07-23 | `/me.html` account area backed by real endpoints: `handleChangePassword` (sets/changes with current-password check, doubles as first-time set for magic-link-only accounts) + `handleDeleteAccount` (clears PII, deliberately preserves order/piece rows for authenticity history) | Account management needed real backend, not just UI; deletion must not orphan piece ownership history since passports depend on it |
| ADR-023 | 2026-07-24 | Admin dashboard extended to read the `Pieces` sheet directly: per-size stock (available/sold), low-stock (≤5) and out-of-stock flags, best-sellers by units sold — all computed server-side in `handleAdmin`, none of it exposed to customer-facing pages | Admin needed real inventory visibility; customer-facing stock policy (no exact numbers) stays untouched since this data never leaves the admin response |
| ADR-024 | 2026-07-24 | **Real production bug found and fixed:** every account-sheet lookup (`handleSignup`, `handleWaitlist`, `handleLogin`, `handleForgot`, `handleReset`, `handleVerify`, `handleAdmin`) used `getActiveSheet()`, which returns whatever tab a human last had open in the Sheets UI — not a fixed target. Pinned all 11 occurrences to `getSheetByName('Accounts')` explicitly | Root-caused a real incident: password resets were returning "success" while silently writing to (or reading from) the wrong sheet, leaving `PasswordHash` empty despite a confirmed-successful API response |
| ADR-025 | 2026-07-24 | **Second real bug found in the same incident:** `api/reset-password.js` was an exact duplicate of `api/forgot-password.js` — same file content, wrong filename. It forwarded `action:'forgot'` and silently dropped the `token`/`password` fields the frontend sent, so `handleReset` was never actually invoked | Explains why the ADR-024 backend fix alone didn't resolve the bug on its own — the request never reached the corrected code path in the first place. Rewrote the proxy to correctly forward `action:'reset', token, password` |
| ADR-026 | 2026-07-24 | Rate limiting added via Apps Script's `CacheService` (no new sheet/infra): `handleLogin` blocks after 5 failed attempts per email per 15 min (successful/unverified responses don't count); `handleForgot` caps at 3 requests per email per 15 min, returning the *same* generic message on limit as on any other outcome | Login had zero brute-force protection; forgot-password had zero anti-spam protection. Fixed-time-bucket cache keys avoid TTL-extension edge cases. Kept the rate-limit response identical to the normal response so hitting the limit can't be used as an email-enumeration oracle |
| ADR-027 | 2026-07-24 | Mobile header rebuilt as hamburger + slide-in side-menu (`shop.html`, `product.html`) below 720px; desktop layout untouched. Account UI (name/avatar/logout) duplicated with separate DOM ids for the mobile panel rather than physically relocated, since the panel is `position:fixed` and moving the original element out of the header flex row would have broken the desktop layout | Real UX complaint with screenshot evidence: 5 elements (cart, logout, name, avatar, nav link) crammed into one row on a 375px screen. Duplicate-with-shared-update-logic was chosen over DOM relocation to avoid a desktop regression |
| ADR-028 | 2026-07-24 | Extracted `assets/products.js` (`window.DIRBALK_PRODUCTS`) as the single source of truth for the 3-product catalog, replacing three independently-maintained copies in `shop.html`, `product.html`, `checkout.html` that had already drifted (different field sets: `desc` vs `story`/`details`/`media`) | A price or stock change required editing three files and could silently desync; now one file, one edit. Validated the merged file by executing it in Node with a stubbed `window` before shipping |
| ADR-029 | 2026-07-24 | Extracted `assets/password-ux.js` (`DirbalkPassword.attachCapsLock`, `.attachStrength`) as a shared module, wired into `signin.html`, `reset-password.html`, `me.html`. Strength meter intentionally omitted from login/confirm/delete-confirmation fields — only shown where a *new* password is actually being chosen | Same duplication-prevention principle as ADR-028, applied proactively this time instead of after drift occurred. Strength-meter placement matches actual UX intent, not "show it everywhere" |
| ADR-030 | 2026-07-24 | Rebuilt all GA4 events (`view_item`, `add_to_cart`, `begin_checkout`, `add_shipping_info`, `purchase`) to the Enhanced Ecommerce schema (`items[]`, `value`, `currency`, `transaction_id` on purchase) instead of ad-hoc custom parameter shapes; added missing `view_item_list`/`select_item`; added GA4-standard `sign_up`/`login` events alongside existing custom ones (additive, not a rename) | Events were firing but invisible to GA4's own built-in funnel/revenue/cart-abandonment reports, which require this exact shape. Chose to feed GA4's native reporting rather than build custom funnel infrastructure |
| ADR-031 | 2026-07-24 | **Real gap found:** `enter.html` (the magic-link drop-entry page) had no Google Analytics loaded at all — the single most important conversion point in the invite funnel was completely untracked. Added the GA4 snippet plus a `login` event with `method:'magic_link'` | Found while auditing the funnel for ADR-030; without this, drop-day conversion from invite → entry was structurally invisible in any report |
| ADR-032 | 2026-07-24 | `PROJECT_ROADMAP.md` and `LAUNCH_CHECKLIST.md` created as living documents, separate from `ARCHITECTURE.md` — roadmap tracks phase-by-phase feature completion honestly (including what's unverifiable without real browser/device access), checklist tracks concrete pre-launch action items split by who owns them (Claude/Fadel) and severity (blocking/should/nice-to-have/operational) | Needed a place to track "is this actually launch-ready" separately from "how is this architected" — conflating the two in `ARCHITECTURE.md` would make both harder to use |
نسخ هالصفوف لآخر جدول §21 بملف ARCHITECTURE.md (بعد ADR-032)

| ADR-033 | 2026-07-24 | handleUpdateOrderStatus (admin-only) added to Apps Script (v24) — updates an order's Status cell and automatically fires a branded email when status becomes 'تم الشحن' or 'تم التسليم'. The delivered email looks up whether a piece was claimed against that order and links straight to /unbox.html?p={slug} for that specific piece instead of a generic page | Connects two already-built systems (order status + piece passports) instead of sending a flat "your order arrived" email; admin UI wired via a kind branch on the existing admin-data.js proxy rather than a new Vercel function (already at 12/12 on Hobby) | | ADR-034 | 2026-07-24 | Order status dropdown in admin.html requires manual confirmation before saving 'تم الشحن' or 'تم التسليم' specifically (the two statuses that trigger a customer email); reverts to the previous value automatically on any failure or cancelled confirm | A stray click shouldn't be able to silently email a customer; the dropdown must never visually show a status that isn't actually true in the sheet | | ADR-035 | 2026-07-24 | Abandoned-cart reminder system (v25): handleLogAbandonedCart logs a candidate row the moment someone submits the checkout shipping step — the earliest point the system has their email + cart contents, since cart state otherwise lives only in browser localStorage and never reaches the backend before payment. A separate checkAbandonedCarts function (installed manually once via Apps Script's Triggers UI — cannot self-install from a web request) checks candidates 2–48h old against completed orders and sends one reminder to anyone who never converted | Real architectural gap: there was no server-side cart visibility at all before this. Logging happens fire-and-forget from checkout.html so a failure never blocks real checkout | | ADR-036 | 2026-07-24 | create-order.js extended with a kind: 'logAbandonedCart' branch rather than a new proxy file; cartItemsSummary() extracted as a shared helper in checkout.html to avoid a second copy of the same items-string logic that already existed at final-order-submission | Same 12-function-limit discipline as ADR-033; minor duplication caught and fixed in the same pass rather than left for later | | ADR-037 | 2026-07-24 | Two real mobile bugs found and fixed together: the cart icon (#cartBtn) had been nested inside .header-actions, which the mobile hamburger retrofit hid entirely — meaning cart was invisible on mobile, and fly-to-cart silently no-op'd (it reads the cart icon's position before animating, and a hidden element has none). Fixed by making #cartBtn a direct sibling in the header row, with order CSS to preserve its exact desktop position | User-reported with screenshots; one root cause explained two separate symptoms | | ADR-038 | 2026-07-24 | product.html's image gallery given explicit prev/next navigation with wraparound at both ends: visible arrow buttons on mobile only (desktop's vertical stack + real keyboard ArrowLeft/ArrowRight cover that case instead, since fixed-position side arrows don't sit well against the 2-column desktop layout) | Gallery previously only had implicit swipe/scroll, no explicit control, unlike the shop grid's cards which already had looping arrows | | ADR-039 | 2026-07-24 | Mobile header rebuilt as a true CSS Grid (1fr auto 1fr) instead of flexbox-with-order-hacks, guaranteeing the logo is mathematically centered regardless of hamburger/cart icon width differences | Flex-based centering only looked centered by coincidence when flanking elements happened to be similar width; grid guarantees it structurally | | ADR-040 | 2026-07-24 | The hamburger + centered-logo + cart-only header made the permanent layout at every screen size, not just mobile — the @media (max-width:720px) restriction was removed entirely from shop.html/product.html. Applied the same pattern site-wide to about.html, drop.html, shipping.html, privacy.html, signin.html, and me.html; deliberately not applied to admin.html, which is an internal tool, not part of the customer-facing site | Explicit user decision to unify navigation everywhere rather than have it differ by screen size or by page. privacy.html and signin.html had no header at all before this — signin.html uses position:fixed for its header instead of sticky since its body is flex-centered for the login card, and a normal in-flow header would have disturbed that centering; privacy.html's old body{display:flex;justify-content:center} centering was moved onto .wrap via margin:auto so a normal header could sit above it without becoming a flex sibling | | ADR-041 | 2026-07-24 | Side menu's "الكوليكشن" entry on shop.html/product.html given an expandable submenu listing all 3 products, built dynamically from window.DIRBALK_PRODUCTS (the shared products.js file) rather than hardcoded a second time | Lets any page jump directly to a specific product without going through the shop grid first; reading from the existing single source of truth means adding a 4th product later requires zero extra menu code |
